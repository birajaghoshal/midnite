"""Code from torchfcn and fcn, without CUDA requirement.

torchfcn (from https://github.com/wkentaro/pytorch-fcn) requires fcn
(https://github.com/wkentaro/fcn), which requires cupy. Cupy in turn requires a CUDA
installation, which is only possible to have on machines with CUDA-supported devices.
However, to use a pretrained model, no cupy calls are required - hence this copy
which can be used without CUDA.
